...
class PhysicsQuestions with ChangeNotifier {
  ...
  PhysicsQuestions(this.token, this.userId, this._items);
  ...
  Future<void> fetchAndSetQuestions() async {
      ...
      var url = Uri.parse(
          'https://examination-4e1aa-default-rtdb.firebaseio.com/physicsQuestions.json?auth=$token');
      try {
  ...
  Future<void> updateQuestion(String? questionId, Question newQuestion) async{
      ...
      if (questionIndex >= 0) {
        final url = Uri.parse(
            "https://examination-4e1aa-default-rtdb.firebaseio.com/physicsQuestions/$questionId.json?auth=$token");
