...
class AdminPhysicsQuestionItem extends StatelessWidget {
  const AdminPhysicsQuestionItem({Key? key, required this.question, required this.id}) : super(key: key);
  final String? id;
              ...
              child: Row(
                children: [
                  IconButton(onPressed: (){
                    Navigator.of(context).pushNamed(EditPhysicsQuestionScreen.namedRoute, arguments: id);
                  }, icon: const Icon(Icons.edit, color: Colors.black54,)),
                  IconButton(onPressed: () async{
                    try{
                      await Provider.of<PhysicsQuestions>(context, listen: false).removeQuestion(id!);
                    }catch(error){
