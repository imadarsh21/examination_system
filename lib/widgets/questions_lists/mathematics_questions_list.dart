...
class MathematicsQuestionsList extends StatefulWidget {
  const MathematicsQuestionsList({Key? key}) : super(key: key);

  @override
  State<MathematicsQuestionsList> createState() => _MathematicsQuestionsListState();
}

class _MathematicsQuestionsListState extends State<MathematicsQuestionsList> {
  int? radioValue;
  ...
  Widget build(BuildContext context) {
    final questionsData = Provider.of<MathematicsQuestions>(context);
            ...
            children: [
              ...
              Padding(
                ...
                child: TextButton(
                    onPressed: () async {
                      var url = Uri.parse(
                          'https://examination-4e1aa-default-rtdb.firebaseio.com/mathematicsResponses/${questionsData.userId}/${questionsData.items[i].id}.json?auth=${questionsData.token}');
                      setState(() {
                        radioValue = null;
                      });
              ...
              ElevatedButton(
                  onPressed: () async {
                    await Provider.of<MathematicsResponses>(context, listen: false)
                        .fetchAndGetMathematicsResponses();
                    getScore();
                    Navigator.of(context).pushNamed(MathematicsSubmittedScreen.namedRoute,
                        arguments: score);
  ...
  void sendResponse(int? value, int i, MathematicsQuestions questionData) async {
    //debugPrint("sendResponses has been initiated");
    setState(() {
      radioValue = value;
    });
    var url = Uri.parse(
        'https://examination-4e1aa-default-rtdb.firebaseio.com/mathematicsResponses/${questionData.userId}/${questionData.items[i].id}.json?auth=${questionData.token}');
  ...
  void getScore() {
    List<Question> quesList =
        Provider.of<MathematicsQuestions>(context, listen: false).items;
    List<MathematicsResponse> resList =
    Provider.of<MathematicsResponses>(context, listen: false).items!;
    int k = 0;
