...
class PhysicsQuestionsList extends StatefulWidget {
  const PhysicsQuestionsList({Key? key}) : super(key: key);

  @override
  State<PhysicsQuestionsList> createState() => _PhysicsQuestionsListState();
}

class _PhysicsQuestionsListState extends State<PhysicsQuestionsList> {
  int? radioValue;
  ...
  Widget build(BuildContext context) {
    final questionsData = Provider.of<PhysicsQuestions>(context);
            ...
            children: [
              ...
              Padding(
                ...
                child: TextButton(
                    onPressed: () async {
                      var url = Uri.parse(
                          'https://examination-4e1aa-default-rtdb.firebaseio.com/physicsResponses/${questionsData.userId}/${questionsData.items[i].id}.json?auth=${questionsData.token}');
                      setState(() {
                        radioValue = null;
                      });
              ...
              ElevatedButton(
                  onPressed: () async {
                    await Provider.of<PhysicsResponses>(context, listen: false)
                        .fetchAndGetPhysicsResponses();
                    getScore();
                    Navigator.of(context).pushNamed(PhysicsSubmittedScreen.namedRoute,
                        arguments: score);
  ...
  void sendResponse(int? value, int i, PhysicsQuestions questionData) async {
    //debugPrint("sendResponses has been initiated");
    setState(() {
      radioValue = value;
    });
    var url = Uri.parse(
        'https://examination-4e1aa-default-rtdb.firebaseio.com/physicsResponses/${questionData.userId}/${questionData.items[i].id}.json?auth=${questionData.token}');
  ...
  void getScore() {
    List<Question> quesList =
        Provider.of<PhysicsQuestions>(context, listen: false).items;
    List<PhysicsResponse> resList =
        Provider.of<PhysicsResponses>(context, listen: false).items!;
    int k = 0;
